...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.questions.update', [$question->id]) }}" enctype="multipart/form-data" id="myForm">
                @method('PUT')
                @csrf
                <div class="row">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('test') ? 'is-invalid' : '' }}"
                                ...
                                @foreach ($tests as $id => $entry)
                                    <option value="{{ $id }}"
                                        {{ (old('test_id') ? old('test_id') : $question->test->id ?? '') == $id ? 'selected' : '' }}>
                                        {{ $entry }}</option>
                                @endforeach
                            </select>
                            @if ($errors->has('test'))
                    ...
                    <div class="col-xl-4 col-lg-4 col-md-4 col-xs-4 col-sm-4 col-4">
                        <div class="form-group">
                            ...
                            <input class="form-control {{ $errors->has('question_text') ? 'is-invalid' : '' }}"
                    ...
                    <div class="col-xl-4 col-lg-4 col-md-4 col-xs-4 col-sm-4 col-4">
                        <div class="form-group">
                            ...
                            <input class="form-control {{ $errors->has('points') ? 'is-invalid' : '' }}" type="number"
                                name="points" id="points" value="{{ old('points', $question->points) }}"
                                step="1">
                ...
                <div class="row mt-2">
                    ...
                    <div class="col-xl-8 col-lg-8 col-md-8 col-sm-8 col-xs-8 col-8">
                        <div class="content">
                            <label class="required" for="option_text">{{ trans('cruds.questionOption.title') }}</label>
                            <input type="hidden" name="old_option_count" id="old_option_count" value="{{ count($question->questionOptions) }}">
                            @foreach ($question->questionOptions as $option)
                                <div class="row" id="row{{ $loop->iteration }}">
                                    <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-xs-6 col-6">
                                        <div class="form-group">
                                            <input
                                                class="form-control option_text {{ $errors->has('option_text') ? 'is-invalid' : '' }}"
                                                type="text" name="option_text{{ $loop->iteration }}" id="option_text{{ $loop->iteration }}"
                                                value="{{ old('option_text', $option->option_text) }}" required>
                                            <input type="hidden" name="option_id{{ $loop->iteration }}" value="{{ $option->id }}">
                                            @if ($errors->has('option_text{{ $loop->iteration }}'))
                                                <div class="invalid-feedback">
                                                    {{ $errors->first('option_text') }}
                                                </div>
                                            @endif
                                            <span class="error" id="option_text{{ $loop->iteration }}_error"></span>
                                            <span
                                        ...
                                        <div class="mt-2">
                                            <label class="switch switch-sm switch-success">
                                                <input type="checkbox" class="switch-input" value="1"
                                                    name="is_correct{{ $loop->iteration }}"
                                                    id="is_correct{{ $loop->iteration }}"
                                    ...
                                    <div class="col-xl-2 col-lg-2 col-md-2 col-sm-2 col-xs-2 col-2">
                                        <div class="d-flex mt-1 justify-content-start">
                                            ...
                                            <a class="btn delete-option px-0"><i
                                                    class='bx bx-minus text-danger fw-bold fs-4'
                                                    id="{{ $loop->iteration }}"
                                                    data-optionid="{{ $option->id }}"></i></a>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
...
@section('scripts')
    ...
    <script>
        let btnCount = 0;
        let rowCount = $('.option_text').length;
        let html = "";
        $(document).on('click', '.add-row', function() {
            ...
            html = `<div class="row" id="row${rowCount}">
                        <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-xs-6 col-6">
                            <div class="form-group">
                                <input class="form-control option_text {{ $errors->has('option_text') ? 'is-invalid' : '' }}"
                                    type="text" name="new_option_text[]" id="new_option_text${btnCount}"
                                    value="{{ old('new_option_text${btnCount}', '') }}" required>
                                @if ($errors->has('new_option_text'))
                                    <div class="invalid-feedback">
                                        {{ $errors->first('new_option_text${btnCount}') }}
                                    </div>
                                @endif
                                <span class="error" id="new_option_text${btnCount}_error"></span>
                                <span
                                    class="help-block">{{ trans('cruds.questionOption.fields.option_text_helper') }}</span>
                            </div>
                        </div>
                        <div class="col-xl-3 col-lg-3 col-md-3 col-sm-3 col-xs-3 col-3">
                            <div class="mt-2">
                                <label class="switch switch-sm switch-success">
                                    <input type="checkbox" class="switch-input" value="1" name="new_is_correct${btnCount}" id="new_is_correct${btnCount}">
                                    <span class="switch-toggle-slider">
                                        <span class="switch-on">
                                            <i class="bx bx-check"></i>
                                        </span>
        ...
        $(document).on('click', '.delete-option', (e) => {
            let row = '#row' + $(e.target).attr('id');
            let optionId = $(e.target).data('optionid');
            let count = $('.option_text').length;
            if (count > 1) {
                Swal.fire({
                    icon: 'question',
                    title: `Are you sure?`,
                    text: `Do you really want to delete this option?`,
                    type: 'warning',
                    showClass: {
                        popup: 'animate__animated animate__bounceIn'
                    },
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: "DELETE",
                            url: "/admin/question-options/" + optionId,
                            headers: {
        ...
        Dropzone.options.questionImageDropzone = {
            ...
            init: function() {
                 @if (isset($question) && $question->question_image)
                    ...
                    var id = {!! $question->question_image->id !!}
                    this.options.addedfile.call(this, file)
                    var file_url = '/storage/' + id + '/' + file.file_name;
                    this.options.thumbnail.call(this, file, file_url)
