        ...
        <div class="col-md-12">
            @can('course_category_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.course-categories.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $courseCategory->id }}">
                                        ...
                                        <td>
                                            @can('course_category_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.course-categories.show', $courseCategory->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('course_category_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.course-categories.edit', $courseCategory->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('course_category_delete')
                                                <form action="{{ route('frontend.course-categories.destroy', $courseCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('course_category_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.course-categories.massDestroy') }}",
